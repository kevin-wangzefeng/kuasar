...
function check_test_framework() {
    ...
    local required_files=(
        "test/e2e/Cargo.toml"
        "test/e2e/src/lib.rs"
        "test/e2e/src/main.rs"
        "test/e2e/src/tests.rs"
    ...
    if ! (cd "${KUASAR_ROOT}/test/e2e" && cargo check --quiet 2>/dev/null); then
...
function check_config_files() {
    ...
    local config_dir="${KUASAR_ROOT}/test/e2e/configs"
    ...
    for config in "${required_configs[@]}"; do
        if [[ ! -f "${config_dir}/$config" ]]; then
            kuasar::log::error "Missing config file: test/e2e/configs/$config"
            return 1
        fi
    done
...
function run_verification() {
    ...
    if [[ ${#failed_checks[@]} -eq 0 ]]; then
        ...
        kuasar::log::info "  3. See test/e2e/README.md for detailed usage"
        return 0
    else
