...
mod runtime_tests {
    ...
    #[tokio::test]
    #[serial]
    async fn test_all_runtimes_sequential() {
        info!("Starting sequential test of all runtimes");
        
        let mut ctx = setup_test_context().await;
        let runtimes = ["runc", "wasm", "resource-slot"];
        ...
        info!("Sequential test of all runtimes completed successfully");
    }
}
